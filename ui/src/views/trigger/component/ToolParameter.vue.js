...
const toolParameterFormRef = ref();
...
const input_field_list = computed(() => {
    ...
    if (props.tool && props.tool.input_field_list) {
        props.tool.input_field_list.forEach((item) => {
            result.push({
                field: item.name,
...
const base_node = computed(() => {
    return (props.tool?.work_flow?.nodes || []).find((n) => n.type === 'tool-base-node');
});
...
for (const [f, index] of __VLS_vFor((__VLS_ctx.input_field_list))) {
    ...
    if (__VLS_ctx.modelValue[f.field]) {
        ...
        const { default: __VLS_15 } = __VLS_13.slots;
        {
            ...
            if (__VLS_ctx.modelValue[f.field] &&
                ...
                // @ts-ignore
                [modelValue, modelValue, modelValue, modelValue, input_field_list, $t, $t, $t, $t, trigger, trigger,];
                var __VLS_20;
            }
            // @ts-ignore
            [];
