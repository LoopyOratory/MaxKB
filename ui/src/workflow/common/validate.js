...
export class WorkFlowInstance {
    ...
    workFlowNodes;
    workflowModel;
    constructor(workflow, workflowModel) {
     ...
     */
    is_valid_start_node() {
        const start_node_list = this.nodes.filter((item) => [WorkflowType.Start, WorkflowType.LoopStartNode].includes(item.id));
     ...
     */
    get_start_node() {
        const start_node_list = this.nodes.filter((item) => [WorkflowType.Start, WorkflowType.LoopStartNode, WorkflowType.ToolStartNode].includes(item.id));
    ...
    is_valid_work_flow() {
        ...
        const notInWorkFlowNodes = this.nodes
            .filter((node) => node.id !== WorkflowType.Start &&
    ...
    is_valid_nodes() {
        for (const node of this.nodes) {
            if (node.type !== WorkflowType.Base &&
...
export class KnowledgeWorkFlowInstance extends WorkFlowInstance {
    is_valid_start_node() {
        const start_node_list = this.workflowModel == WorkflowMode.Knowledge
    ...
    is_valid_work_flow() {
        ...
        const notInWorkFlowNodes = this.nodes
            .filter((node) => node.id !== WorkflowType.KnowledgeBase &&
            node.type !== WorkflowType.LoopStartNode &&
    ...
    is_valid_nodes() {
        for (const node of this.nodes) {
            if (node.type !== WorkflowType.KnowledgeBase &&
                node.type !== WorkflowType.LoopStartNode &&
                node.properties.kind !== WorkflowKind.DataSource) {
                if (!this.edges.some((edge) => edge.targetNodeId === node.id)) {
                    throw `${t('workflow.validate.notInWorkFlowNode')}:${node.properties.stepName}`;
    ...
    _get_end_nodes(startNode, value) {
            ...
            if (edge_list.length == 0 && !end.includes(node.type)) {
                if (node.type == WorkflowType.LoopNode) {
                    if (node.properties.node_data.loop_body) {
                        const end_nodes = new KnowledgeWorkFlowInstance(node.properties.node_data.loop_body, WorkflowMode.KnowledgeLoop).get_end_nodes();
