...
class AppNode extends HtmlResize.view {
    ...
    constructor(props, VueNode) {
        ...
        if (props.model.properties.noRender) {
            ...
            props.model.properties.stepName = this.getNodeName(props.graphModel.nodes.filter((node) => node.id !== props.model.id), props.model.properties.stepName);
        }
    ...
    getNodeName(nodes, baseName) {
        ...
        while (true) {
            ...
            if (!nodes.some((node) => node.properties.stepName === name.trim())) {
                return name;
            }
            index++;
        }
    ...
    get_node_field_list() {
        ...
        if (this.props.model.type === 'knowledge-base-node') {
            ...
            if (this.props.model.properties.config?.globalFields) {
                globalFields = this.props.model.properties.config.globalFields.map((item) => ({
    ...
    get_up_node_field_dict(contain_self, use_cache) {
    ...
    get_up_node_field_list(contain_self, use_cache) {
        const result = Object.values(this.get_up_node_field_dict(contain_self, use_cache)).reduce((pre, next) => [...pre, ...next], []);
        const start_node_field_list = (this.props.graphModel.getNodeModelById('start-node') ||
            this.props.graphModel.getNodeModelById('loop-start-node'))?.get_node_field_list() || [];
    ...
    clear_next_node_field(contain_self) {
    ...
    renderVueComponent(root) {
        ...
        if (root) {
            if (isActive()) {
                connect(this.targetId(), this.component, root, model, graphModel, undefined, this.props.graphModel.get_provide);
            }
            else {
...
class AppNodeModel extends HtmlResize.model {
    ...
    setAttributes() {
        ...
        const isLoop = (node_id, target_node_id) => {
        ...
        this.sourceRules.push({
            ...
            validate: (sourceNode, targetNode, sourceAnchor, targetAnchor) => {
                ...
                const is_c = up_edge_list.find((up_edge) => up_edge.targetAnchorId == targetAnchor.id && up_edge.sourceAnchorId == sourceAnchor.id);
