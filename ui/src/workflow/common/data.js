...
export const getMenuNodes = (workflowMode) => {
...
export const workflowModelDict = {
    [WorkflowMode.Application]: (node) => {
        return (['application-node', 'tool-workflow-lib-node', 'tool-lib-node'].includes(node.type) &&
            node?.properties?.node_data?.tool_type !== 'DATA_SOURCE');
    },
    [WorkflowMode.ApplicationLoop]: (node) => {
        return (['application-node', 'tool-workflow-lib-node', 'tool-lib-node'].includes(node.type) &&
            node?.properties?.node_data?.tool_type !== 'DATA_SOURCE');
    },
    [WorkflowMode.Knowledge]: (node) => {
    ...
    [WorkflowMode.KnowledgeLoop]: (node) => {
        return (['tool-workflow-lib-node', 'tool-lib-node'].includes(node.type) &&
            node?.properties?.node_data?.tool_type !== 'DATA_SOURCE');
    },
    [WorkflowMode.Tool]: (node) => {
        return (['tool-workflow-lib-node', 'tool-lib-node'].includes(node.type) &&
            node?.properties?.node_data?.tool_type !== 'DATA_SOURCE');
    },
    [WorkflowMode.ToolLoop]: (node) => {
        return (['tool-workflow-lib-node', 'tool-lib-node'].includes(node.type) &&
            node?.properties?.node_data?.tool_type !== 'DATA_SOURCE');
    },
};
...
function defineLocaleGetter(target, prop, key, fallback) {
    let overrideValue;
    Object.defineProperty(target, prop, {
        configurable: true,
        enumerable: true,
        get: () => overrideValue || t(key, fallback),
        set: (value) => {
            overrideValue = value || undefined;
        },
    });
}
function bindLocale(target, path, key, fallback) {
...
function bindFieldLabels(fields, keys) {
    const refresh = () => {
...
function bindNodeLocale(node, textKey, labelKey, stepNameKey = labelKey) {
...
defineLocaleGetter(toolWorkflowLibNode, 'text', 'workflow.nodes.toolWorlflowNode.text', 'Workflow Tool');
defineLocaleGetter(toolWorkflowLibNode, 'label', 'workflow.nodes.toolWorlflowNode.label', 'Workflow Tool');
bindLocale(toolWorkflowLibNode, 'properties.stepName', 'workflow.nodes.toolWorlflowNode.label', 'Workflow Tool');
...
bindLocale(loopNode, 'properties.workflow.nodes.0.properties.stepName', 'workflow.nodes.startNode.label');
{
    let overrideValue;
...
].forEach(([fields, keys]) => bindFieldLabels(fields, keys));
[
    [
        knowledgeMenuNodes,
        [
...
].forEach(([nodes, keys]) => bindFieldLabels(nodes, keys));
[
