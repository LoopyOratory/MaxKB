...
function removeFromExposed(fieldName) {
    const setting = props.nodeModel.properties.user_input_field_list_setting;
    if (setting?.exposed_fields?.includes(fieldName)) {
        setting.exposed_fields = setting.exposed_fields.filter((f) => f !== fieldName);
    }
}
function openChangeTitleDialog() {
    UserInputTitleDialogRef.value.open(inputFieldConfig.value, inputFieldList.value, props.nodeModel.properties.user_input_field_list_setting, props.nodeModel.properties.user_input_config?.title);
}
...
function refreshFieldTitle(setting) {
    if (setting) {
...
var __VLS_16;
var __VLS_17;
if (props.nodeModel.properties.user_input_field_list?.length > 0) {
    let __VLS_26;
